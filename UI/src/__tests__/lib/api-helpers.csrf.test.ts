...
describe('apiHandler — CSRF default-on', () => {
  it('rejects POST without a CSRF token', async () => {
    ...
    const req = global.createMockRequest({ method: 'POST', headers: {}, body: {}, cookies: {} }) as NextApiRequest
  ...
  it('accepts POST with a valid CSRF token in header and matching cookie', async () => {
    const handler = apiHandler({
      POST: async (_req, res) => {
        res.status(200).json({ ok: true })
      },
