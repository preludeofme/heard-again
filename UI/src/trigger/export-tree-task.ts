...
async function fetchPeopleForExport(familyspaceId: string): Promise<FetchResult> {
  ...
  const familiesByPersonId = new Map<string, { isParent: boolean; isChild: boolean; unit: typeof allFamilyUnits[number] }[]>()
  ...
  const people = allPeople.map((person) => {
    ...
    for (const { isParent, isChild, unit } of families) {
      if (isParent) {
        for (const p of unit.parents) {
          if (!p.parent || p.parentId === person.id || !allIds.has(p.parentId)) continue
          edges.push({
      ...
      if (isChild) {
        ...
        for (const c of unit.children) {
          if (!c.child || c.childId === person.id || !allIds.has(c.childId)) continue
          edges.push({
    ...
    const sexMap: Record<string, 'M' | 'F' | 'U' | 'X'> = { M: 'M', F: 'F', U: 'U', X: 'X' }
  ...
  for (const person of allPeople) {
    if (person.avatarAssetId) avatarAssetIds.set(person.id, person.avatarAssetId)
  }
...
async function uploadToR2(
  ...
  // Use || not ?? — an empty string R2_PUBLIC_URL_BASE should fall through to the next option.
  const publicBase =
    process.env.R2_PUBLIC_URL_BASE || process.env.R2_PUBLIC_DOMAIN

  const accountId = process.env.CLOUDFLARE_ACCOUNT_ID
  const key = `exports/${fileName}`
  await r2.client.send(
    new PutObjectCommand({ Bucket: r2.bucket, Key: key, Body: buffer, ContentType: contentType }),
  )

  const base = publicBase || (accountId ? `https://pub-${accountId}.r2.dev` : '')
  return `${base}/${key}`
}
...
async function writeLocalExport(buffer: Buffer, fileName: string): Promise<string> {
  const exportDir =
...
export const exportTreeTask = task({
  ...
  run: async (payload: ExportTreePayload): Promise<ExportTreeOutput> => {
    ...
    logger.info('Starting family tree export', { familyspaceId, rootPersonId, format })

    // 1. Fetch all people with relationships
    const { people, avatarAssetIds } = await fetchPeopleForExport(familyspaceId)
    ...
    // 2. Determine root
    const rootId = (rootPersonId && people.some((p) => p.id === rootPersonId))
      ? rootPersonId
      : people[0]!.id

    // 3. Layout
    const { nodes, edges } = buildFamilyTreeLayout(people, rootId, DUMMY_CALLBACKS, null, null)
    ...
    if (r2 && !compact) {
      const nodesWithAvatars = Array.from(avatarAssetIds.entries()).map(([nodeId, avatarAssetId]) => ({
        nodeId,
        avatarAssetId,
      }))

      if (nodesWithAvatars.length > 0) {
        logger.info(`Fetching ${nodesWithAvatars.length} avatars directly from R2`)
      ...
      logger.info(`Compact mode (${nodeCount} nodes > ${COMPACT_NODE_THRESHOLD}) — skipping avatars`)
    }
    ...
    for (const [nodeId, { buffer, mimeType }] of avatarBuffers) {
      avatarDataUrls.set(nodeId, `data:${mimeType};base64,${buffer.toString('base64')}`)
    }
    ...
    if (!useLocal && !r2) throw new Error('R2 credentials required for export upload')
    ...
    const pngBuffer = await sharp(Buffer.from(renderSvg), { density: 72, limitInputPixels: false })
      .png({ compressionLevel: 8 })
      .toBuffer()

    logger.info(`PNG rasterized (${Math.round(pngBuffer.length / 1024 / 1024 * 10) / 10} MB)`)
