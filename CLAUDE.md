...
Heard Again is a multi-service application for preserving family stories through voice, text, and media. It combines a primary Next.js web app and a Python TTS voice synthesis service.

> The AI chat system (RAG + LLM persona chat) has been removed from this branch and is being developed separately; it will be reintroduced later.
...
- `docker-compose.yml` — production; services with GPU profile (`with-tts`)
...
- **Unit tests**: Jest in `UI`, colocated in `__tests__/` directories
