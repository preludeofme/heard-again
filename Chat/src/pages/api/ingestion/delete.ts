...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!assetId || !workspaceId) {
    return res.status(400).json({ error: 'Missing required fields: assetId, workspaceId' })
  }
  ...
  try {
    ...
    if (chunkIds.length > 0) {
      ...
      const collectionName = `workspace_${workspaceId}_documents`
      ...
      logger.info({ assetId, workspaceId, documentId: document.id, chunkCount: chunkIds.length }, 'Removed document chunks from ChromaDB')
    }
  } catch (error) {
    logger.warn({ assetId, workspaceId, err: error instanceof Error ? error.message : String(error) }, 'Failed removing chunks from ChromaDB; continuing with DB delete')
  }

  await (prisma as any).document.delete({ where: { id: document.id } })
  logger.info({ assetId, workspaceId, documentId: document.id }, 'Removed document from Chat DB')
